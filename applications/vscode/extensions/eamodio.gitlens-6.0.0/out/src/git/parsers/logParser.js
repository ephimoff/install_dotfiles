...
class GitLogParser {
                ...
                case 'filename':
                    if (type === git_1.GitCommitType.Branch) {
                        ...
                        if (entry.fileStatuses) {
                            entry.fileName = system_1.Arrays.filterMap(entry.fileStatuses, f => !!f.fileName ? f.fileName : undefined).join(', ');
                        }
                    }
                    else {
                        next = lines.next();
    ...
    static parseEntry(entry, type, repoPath, relativeFileName, commits, authors, recentCommit) {
