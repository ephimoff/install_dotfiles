...
class RepositoryNode extends explorerNode_1.ExplorerNode {
    ...
    getTreeItem() {
        this.updateSubscription();
        const item = new vscode_1.TreeItem(`Repository ${system_1.Strings.pad(constants_1.GlyphChars.Dash, 1, 1)} ${this.repo.name || this.uri.repoPath}`, vscode_1.TreeItemCollapsibleState.Expanded);
    ...
    onRepoChanged(e) {
        ...
        let node;
        if (this.children !== undefined && e.changed(gitService_1.RepositoryChange.Stashes, true)) {
            node = this.children.find(c => c instanceof stashesNode_1.StashesNode);
        }
