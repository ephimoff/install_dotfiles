...
class GitService extends vscode_1.Disposable {
    ...
    get repoPath() {
        if (this._repositories.size !== 1)
            return undefined;
        const repo = system_1.Iterables.first(this._repositories.values());
    ...
    onWorkspaceFoldersChanged(e) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const f of e.added) {
                ...
                const rp = yield this.getRepoPathCore(fsPath, true);
                if (rp === undefined) {
                    logger_1.Logger.log(`onWorkspaceFoldersChanged(${fsPath})`, 'No repository found');
                    this._repositories.set(fsPath, undefined);
                }
                else {
                    this._repositories.set(fsPath, new git_1.Repository(f, rp, this, this.onAnyRepositoryChanged.bind(this), this._suspended));
                }
            }
            ...
            const hasRepository = system_1.Iterables.some(this._repositories.values(), rp => rp !== undefined);
            yield constants_1.setCommandContext(constants_1.CommandContext.HasRepository, hasRepository);
            if (!initializing) {
    ...
    getRepoPath(filePathOrUri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof filePathOrUri === 'string')
                return this.getRepoPathCore(filePathOrUri, false);
    ...
    getRepoPathCore(filePath, isDirectory) {
        return git_1.Git.revparse_toplevel(isDirectory ? filePath : path.dirname(filePath));
    }
    getRepositories() {
        return __awaiter(this, void 0, void 0, function* () {
            const repositories = yield this.getRepositoriesCore();
            return [...system_1.Iterables.filter(repositories.values(), r => r !== undefined)];
        });
    }
    getRepositoriesCore() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this._repositoriesPromise !== undefined) {
                yield this._repositoriesPromise;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (repoPathOrUri instanceof gitUri_1.GitUri) {
                repoPathOrUri = repoPathOrUri.repoPath !== undefined
                    ? vscode_1.Uri.file(repoPathOrUri.repoPath)
                    : repoPathOrUri.fileUri();
            }
            if (typeof repoPathOrUri === 'string') {
                const repositories = yield this.getRepositoriesCore();
                return system_1.Iterables.find(repositories.values(), r => r !== undefined && r.path === repoPathOrUri) || undefined;
            }
            const folder = vscode_1.workspace.getWorkspaceFolder(repoPathOrUri);
            if (folder === undefined)
                return undefined;
            const repositories = yield this.getRepositoriesCore();
    ...
    openDiffTool(repoPath, uri, staged) {
        logger_1.Logger.log(`openDiffTool('${repoPath}', '${uri.fsPath}', ${staged})`);
